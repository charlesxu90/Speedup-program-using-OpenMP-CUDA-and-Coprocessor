...
#pragma acc data copy(coef[0:5],U3[0:p.domain_size],source[0:nt]) copy(tempU1[0:p.domain_size],tempU2[0:p.domain_size])
    for (i=0; i<nt/2; i+=2) { // time loop
        //printf("Time step: %d\n", i);
#pragma acc kernels
#pragma acc loop independent gang(256) collapse(3)// private(x,y,z,lap)
        for(x=4; x< p.ip-4; x++) {
            for(y=4; y< p.jp-4; y++) {
                for(z=4; z< p.kp-4; z++) {
                    lap=coef[0]*Vc(x,y,z)
...
#pragma acc kernels
#pragma acc loop  independent collapse(3) gang(256)// private(x,y,z,lap)
        for(x=4; x< p.ip-4; x++) {
            for(y=4; y< p.jp-4; y++) {
                for(z=4; z< p.kp-4; z++) {
                    lap=coef[0]*Uc(x,y,z)
